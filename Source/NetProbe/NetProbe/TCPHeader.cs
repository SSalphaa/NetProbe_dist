...
namespace NetProbe
{
    public class TCPHeader
    {
        ...
        //End Encapsulated TCP Data fields
        
        //End Encapsulated IP Data

        public TCPHeader(byte [] byBuffer, int nReceived)
        {
            try
            {
                //create memory stream out of the received TCP packets
                MemoryStream memoryStream = new MemoryStream(byBuffer, 0, nReceived);
                //Binary reader allows to read the indicated number of bits from the memory stream and advances the current position of the stream
            ...
            {
                MessageBox.Show(ex.Message, "NetProbe TCP" + (nReceived), MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
